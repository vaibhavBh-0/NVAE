...
class Normal:
    ...
    @staticmethod
    def from_product_of_normals(normals: List['Normal']):
        """
        From "Products and Convolutions of Gaussian Probability Density Functions" - P.A. Bromiley
        http://in.ruc.edu.cn/wp-content/uploads/2016/09/The-product-and-convolution-of-guassian-distributions.pdf
        """

        means = torch.stack([normal.mu for normal in normals])
        vars = torch.stack([normal.sigma for normal in normals]) ** 2
        eps = 1e-6

        new_var = (vars ** -1).sum(dim=0) ** -1
        new_mean = new_var * (means / (vars + eps)).sum(dim=0)

        new_log_sigma = new_var.log() / 2

        return Normal(mu=new_mean, log_sigma=new_log_sigma)
...
class DiscLogistic:
    ...
    def sample(self):
        ...
        u = torch.Tensor(self.means.size()).uniform_(1e-5, 1. - 1e-5).to(device=self.device)                        # B, 3, H, W
...
class DiscMixLogistic:
    ...
    def sample(self, t=1.):
        gumbel = -torch.log(- torch.log(torch.Tensor(self.logit_probs.size()).uniform_(1e-5, 1. - 1e-5).to(device=self.device)))  # B, M, H, W
        sel = one_hot(torch.argmax(self.logit_probs / t + gumbel, 1), self.num_mix, dim=1)  # B, M, H, W
        sel = sel.unsqueeze(1).to(device=self.device)  # B, 1, M, H, W
        ...
        u = torch.Tensor(means.size()).uniform_(1e-5, 1. - 1e-5).to(device=self.device)      # B, 3, H, W
